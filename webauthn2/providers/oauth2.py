...
from oauth2client.crypt import AppIdentityError, CLOCK_SKEW_SECS, AUTH_TOKEN_LIFETIME_SECS, MAX_TOKEN_LIFETIME_SECS, \
    PyCryptoVerifier
import time
import base64
import Cryptodome
...
""" % dict(
    ...
    def update_timeout(self, conn, cur, force=False):
        ...
        def db_body(conn, cur):
            cur.execute("delete from {referrer_table} where timeout < now()".format(
...
""" % (dict(
    ...
    def log_referrer(self, nonce, referrer, conn, cur):
        def db_body(conn, cur):
            cur.execute(
                "insert into {referrer_table}(nonce, referrer, timeout) select {nonce}, {referrer}, now() + interval '{referrer_timeout} seconds'" \
...
""".format(
        ...
        if len(rows) != 1:
            raise ValueError(
                "Found referrer {x} rows for nonce {nonce}; expected 1".format(x=str(len(rows)), nonce=nonce))
...
class OAuth2Login(ClientLogin):
    ...
    def login(self, manager, context, conn, cur, **kwargs):
        ...
        if self.userinfo.get('active') != True or self.userinfo.get('iss') == None or self.userinfo.get('sub') == None:
            deriva_debug(
                "Login failed, userinfo is not active, or iss or sub is missing: {u}".format(u=str(self.userinfo)))
            raise OAuth2UserinfoError(
                "Login failed, userinfo is not active, or iss or sub is missing: {u}".format(u=str(self.userinfo)))
        ...
        # Update user table
        self.create_or_update_user(manager, context, username, self.id_token, self.userinfo, base_timestamp,
                                   self.payload, conn, cur)
    ...
    def authorization_code_flow(self, context, conn, cur):
        ...
        if not self.provider.nonce_state.hash_matches(nonce_vals['auth_cookie_nonce'], nonce_vals['auth_url_nonce'],
                                                      conn, cur):
        ...
        token_request = urllib.request.Request(self.provider.cfg.get('token_endpoint'),
                                               urllib.parse.urlencode(token_args).encode())
        ...
        if raw_id_token is None:
            deriva_debug(
                "Illegal token response: didn't include an id token. Keys were {k}. Token type was {t}, scope was {s}".format(
                    k=str(self.payload.keys()), t=str(self.payload.get('token_type')),
                    s=str(self.payload.get('scope'))))
    ...
    def create_or_update_user(self, manager, context, username, id_token, userinfo, base_timestamp, token_payload, conn,
                              cur):
        ...
        if token_payload.get('exp') != None:
            ...
            context.user['access_token_expiration'] = base_timestamp + timedelta(
                seconds=int(token_payload.get('expires_in')))
        ...
        for retry in range(1, max_retries):
            try:
                ...
                deriva_debug("Attempt {x} of {y} failed: Got {t} exception {ev} while {text} (url {url})".format(
                    x=str(retry), y=str(max_retries), t=str(type(ev)), ev=str(ev), text=str(text),
        ...
        if 'openid' in found_scopes and self.id_token != None:
            ...
            for key in ['iss', 'aud']:
                ...
                if not (uval == ival or (isinstance(uval, list) and ival in uval)):
                    ...
                    deriva_debug("userinfo[{key}] = {u}, id[{key}] = {i}".format(key=key, u=str(self.userinfo.get(key)),
                                                                                 i=str(self.id_token.get(key))))
    ...
    def validate_access_token(cls, alg, expected_hash, access_token):
        ...
        if hash != expected_hash:
            ...
            if hash != expected_hash:
                raise OAuth2Exception("Bad hash value in access token, alg = '" + alg + "', hash = '" + str(
                    hash) + "', expected = '" + str(expected_hash) + "'")
...
class OAuth2PreauthProvider(PreauthProvider):
    ...
    def preauth_initiate(self, manager, context, conn, cur, do_redirect):
        ...
        deriva_ctx.deriva_response.set_cookie(self.nonce_cookie_name, session.get('auth_cookie_nonce'), secure=True,
                                              path="/")
        ...
        if discovery_scopes is not None:
            ...
            for key in discovery_scopes.keys():
                if discovery_scopes[key] in accepted_scopes:
                    ...
                    deriva_debug("'{s}' is configured as a discovery scope but not an accepted scope".format(
                        s=discovery_scopes[key]))
