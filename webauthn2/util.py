...
def get_wallet_entries(wallet, credential_type, args={}):
    ...
    #
    # For example:
    # get_wallet_entries(client.wallet.extra_values, "oauth2", {"resource_server": "identifiers.globus.org"})
    # returns a list of oauth2 credentials associated with the resource server identifiers.globus.org
    #
    # get_wallet_entries(client.wallet.extra_values, "oauth2", {"credential_source": "auth.globus.org"})
    # returns a list of oauth2 credentials obtained from auth.globus.org
    #
    # get_wallet_entries(client.wallet.extra_values, "oauth2",
    ...
    if args.get('credential_source') != None:
        base = [wallet.get(args.get('credential_source'))]
    else:
        base = wallet.values()
